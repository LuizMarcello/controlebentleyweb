...
@section('content')
    <div class="container">
        <div class="row">
            <div class="col-md-9">
                {{-- <a href="{{ route('empresa.relatorios.modelocontratos') }}" class="btn btn-primary btn-sm">Relatório de modelocontratos</a> --}}
                <div class="card">
                    <div class="card-header">
                        <h4> Modelos de contratos</h4>
                    </div>
                    <div class="card-body">
                        <a href="{{ url('/modelocontratos/create') }}" class="btn btn-success btn-sm" title="Adicionar novo modelo de contrato">
                                    ...
                                    @foreach ($modelocontratos as $modelocontratos)
                                        <tr>
                                           {{--  <td>{{ $loop->iteration }}</td> --}}
                                            <td>{{ $modelocontratos->descricao }}</td>
                                            
                                            <td>
                                                <a href="{{ url('/modelocontratos/' . $modelocontratos->id) }}" title="View modelocontratos">
                                                    ...
                                                    {{-- Parâmetros: Nome do gate e instância do modelocontratos, o qual terá ou não permissão. --}}
                                                    @can('update-client',  $modelocontratos)
                                                    <a href="{{ url('/modelocontratos/' . $modelocontratos->id . '/edit') }}" title="Edit modelocontratos">
                                                        <button class="btn btn-primary btn-sm">
                                                            <i class="fa fa-pencil-square-o" aria-hidden="true"></i>Editar
                                                        </button></a>

                                                        <form method="POST" action="{{ url('/modelocontratos' . '/' . $modelocontratos->id) }}"
                                                            accept-charset="UTF-8" style="display:inline">
                                                            {{ method_field('DELETE') }}
                                                            {{ csrf_field() }}
                                                            <button type="submit" class="btn btn-danger btn-sm"
                                                                title="Delete modelocontratos"
