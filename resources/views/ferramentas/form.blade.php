<div class="form-group {{ $errors->has('categoria') ? 'has-error' : '' }}">
    ...
    <select name="categoria" class="form-control" id="categoria" required>
        @foreach (json_decode('{" ferramenta":"Ferramenta","instrumento":"Instrumento"}',
...
<div class="form-group {{ $errors->has('tipoferramenta') ? 'has-error' : '' }}">
    ...
    <select name="tipoferramenta" class="form-control" id="tipoferramenta" required>
        @foreach (json_decode('{" chavedefenda":"Chave de fenda","chavephilips":"Chave Phillips","alicatedebico":"Alicate de bico","alicatedecorte":"Alicate de corte",
        "alicateuniversal":"Alicate universal","chaveinglesa":"Chave Inglesa","multimetro":"Multimetro"}', true) as $optionKey=>$optionValue)
        <option value="{{ $optionKey }}"
...
<div class="form-group {{ $errors->has('unidademedida') ? 'has-error' : '' }}">
    ...
    <select name="unidademedida" class="form-control" id="unidademedida">
        @foreach (json_decode('{"polegadas":"Polegadas","milimetros":"Milimetros","centimetros":"Centimetros"}', true) as $optionKey=>
            $optionValue)
            <option value="{{ $optionKey }}"
...
<div class="form-group {{ $errors->has('desde') ? 'has-error' : '' }}">
    ...
    <input class="form-control" name="desde" type="text" id="desde"
...
<div class="form-group {{ $errors->has('observacao') ? 'has-error' : '' }}">
    ...
    <textarea class="form-control" rows="5" name="observacao" type="textarea"
        id="observacao">{{ isset($ferramenta->observacao) ? $ferramenta->observacao : '' }}</textarea>
